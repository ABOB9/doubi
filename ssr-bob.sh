...
SSR_installation_status(){
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} not found config file of ssr:没有发现 ShadowsocksR 配置文件，请检查 !" && exit 1
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error} not found ssr folder:没有发现 ShadowsocksR 文件夹，请检查 !" && exit 1
}
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		echo -e " hunxiao\t    : ${Red_font_prefix}${obfs}${Font_color_suffix}"
		echo -e " device amount limit : ${Green_font_prefix}${protocol_param}${Font_color_suffix}"
		echo -e " speed_limit_per_Thread : ${Green_font_prefix}${speed_limit_per_con} KB/S${Font_color_suffix}"
		echo -e " speed_limit_per_user : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}"
		echo -e "${ss_link}"
		echo -e "${ssr_link}"
 ...
 协议和混淆后面的[ _compatible ]，指的是 兼容原版协议/混淆。"
		...
		echo -e " device amount limit : ${Green_font_prefix}${protocol_param}${Font_color_suffix}"
		echo -e " speed_limit_per_Thread : ${Green_font_prefix}${speed_limit_per_con} KB/S${Font_color_suffix}"
		echo -e " speed_limit_per_user : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}" && echo
...
Set_config_port(){
	while true
	do
	echo -e "please input ShadowsocksR port"
...
Set_config_password(){
	echo "please input ShadowsocksR password"
...
Set_config_method(){
	echo -e "please select a encrypt method for ShadowsocksR
	
 ${Green_font_prefix} 1.${Font_color_suffix} none
 ${Tip} if you use auth_chain_a protocol，please select none，混淆随意(建议 plain)
...
Set_config_protocol(){
 ...
 ${Tip} if you use auth_chain_a protocol，please select none for encrypt method，混淆随意(建议 plain)" && echo
...
Set_config_obfs(){
	echo -e "please select hunxiao plugin
